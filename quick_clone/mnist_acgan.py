# https://keras.io/zh/examples/mnist_acgan/
...
def build_generator(latent_size):
    ...
    cnn_block = keras.Sequential([
        ...
        # upsample to (7, 7, ...)
        keras.layers.Conv2DTranspose(192, 5, strides=1, padding='valid', activation='relu', kernel_initializer='glorot_normal'),
        keras.layers.BatchNormalization(),
        # upsample to (14, 14, ...)
        keras.layers.Conv2DTranspose(96, 5, strides=2, padding='same', activation='relu', kernel_initializer='glorot_normal'),
        keras.layers.BatchNormalization(),
        # upsample to (28, 28, ...)
        keras.layers.Conv2DTranspose(1, 5, strides=2, padding='same', activation='tanh', kernel_initializer='glorot_normal'),
    ])

    # this is the z space commonly referred to in GAN papers
    latent_input = keras.layers.Input(shape=[latent_size])

    # this will be our label
    label_input = keras.layers.Input(shape=[1], dtype='int32')
    label_branch = keras.layers.Embedding(input_dim=num_classes, output_dim=latent_size, input_length=1, embeddings_initializer='glorot_normal')(label_input)
    ...
    fake_image = cnn_block(main_branch)

    return keras.Model([latent_input, label_input], fake_image)
...
def build_discriminator():
    ...
    main_branch = cnn_block(image_input)
    pred_validity = keras.layers.Dense(1, activation='sigmoid', name='generation')(main_branch)
    pred_label = keras.layers.Dense(num_classes, activation='softmax', name='auxiliary')(main_branch)

    return keras.Model(image_input, [pred_validity, pred_label])
